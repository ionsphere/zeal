...
Forced completion validation is now enforced by the core agent loop. A completion claim triggers a separate tool-free model call comparing the result and transcript evidence against the user request. Gaps return to the worker for bounded correction and another review; blockers and exhausted steps or attempts fail explicitly. This applies to top-level agents and individual swarm roles. Validation of a swarm's assembled result, artifact revision binding, configurable validation models, and total run budgets remain planned. See [the loop contract](CAPABILITIES.md#forced-completion-validation-loop).

1. Capability/action and completion contracts: mandatory completion recall with bounded corrective loops is active in the core agent loop; versioned support reports, scoped requirements, typed actions, authority inheritance, and fail-closed negotiation remain pending. Initial declarations are in [CAPABILITIES.md](CAPABILITIES.md).
